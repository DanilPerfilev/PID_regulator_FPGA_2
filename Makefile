NAME = PID
VERILOG += top.sv
VERILOG += Period_time.sv
VERILOG += strobe_gen.sv
VERILOG += tm1638_board.sv
VERILOG += display.sv
VERILOG +=divfunc.v
...
$(NAME).bit: $(LPF) $(VERILOG)
	yosys -p "verilog_defaults -add -I$(subst :, -I,$(INC))" -p "synth_ecp5 -noabc9 -json $(NAME).json -top top" $(VERILOG) -l $(NAME).yosys.log 
